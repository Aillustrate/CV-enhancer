...
class CVEnhancer:
    ...
    def make_prompt(self, cv_path, job_path):
        ...
        return self.TEMPLATE.format(system_prompt=self.system_prompt, user_message=user_prompt, assistant_message=self.ASSISTANT_MESSAGE_START)
    
    def generate_text(self, text, **generation_kwargs):
            input_ids = self.tokenizer(text, return_tensors='pt').input_ids.to('cuda')
            output = self.tokenizer.decode(
                self.model.generate(input_ids, pad_token_id=self.tokenizer.eos_token_id, 
    ...
    def save_report(self, llm_output, cv_path, job_path, **generation_kwargs):
        ...
        new_report = {'cv': cv_path, 'job': job_path, 'system_prompt': self.system_prompt, **generation_kwargs, 'model':MODEL_NAME, 'output':llm_output}
        report_id = make_id()
